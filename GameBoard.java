...
public class GameBoard extends JPanel implements Runnable, MouseListener, MouseMotionListener {
    ...
    private int panelStuff = 7;// 0 = nothing,1 = Start of game,2 = draw tickets,3 = Inventory,4 = Gambling,5 = Grey route payment,6 = Double route payment,7 = choosing color for grey Gambling
...
//        	g2d.fillRect(0, 0, getWidth(), getHeight());
    ...
    public void mouseClicked(MouseEvent e) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if( x>=1539 && x<=1612 && y>=481 && y<=595 && panelStuff == 7  )
			{
				//change X-locations of all cards
			}
			if( x>=1666-15 && x<=1766-35 && y>=481 && y<=595 && panelStuff == 7  )
			{
				
			}
			if( x>=1766-35 && x<=1866-55 && y>=481 && y<=595 && panelStuff == 7  )
			{
				
			}
			if( x>=1866-55 && x<=1966-55 && y>=481 && y<=595 && panelStuff == 7  )
			{
				
			}
			//2nd row of cards
			if( x>=1566 && x<=1666-15 && y>=684 && y<=805 && panelStuff == 7  )
			{
